...
function emitTs(series, meta, names, indexes) {
  ...
  lines.push(`// 覆盖标的：${codes.length} 只（成交额排行榜前 ${RANK_TOP} + 旧快照保留）+ 大盘指数 ${indexes.size} 个`);
  ...
  lines.push("};");
  lines.push("");
  lines.push("// 指数代码 -> 名称（大盘行情面板 / 基准对比用）");
  lines.push("export const REAL_INDEX_NAMES: Record<string, string> = {");
  for (const [sym, name] of indexes) {
    lines.push(`  "${sym}": "${name}",`);
  }
  lines.push("};");
  lines.push("");
  lines.push("// 指数日K：code -> [[date, open, close, high, low, volume], ...]（正序，不复权点位）");
  lines.push("export const REAL_INDEXES: Record<string, Array<[string, number, number, number, number, number]>> = {");
