...
- ⚠️ **混合 A股 + 加密时的日历效应（已知限制）**：并集日历按加密口径（每天都有 K 线）展开，
  A股非交易日沿用上一根收盘价。于是「60 个快照日」里 A股只有约 42 根真实 K 线，其余是重复值，
  会平滑 A股的日内波动、削弱动量类信号。**同市场组合（纯 A股 或 纯加密）不受影响**，
  跨市场对冲/轮动场景请知晓这一口径。TODO(v2)：支持按 A股交易日历重采样。
